...
class _SelectModelState extends State<SelectModel> {
  ...
  Future<void> _loadFilePath() async {
    ...
    setState(() {
      _tfliteFilePath = prefs.getString('tfliteFilePath');
      _labelFilePath = prefs.getString('labelFilePath');
    });
  }
  ...
  Future<void> _pickTfliteFile() async {
    ...
    if (result != null) {
      ...
      if (filePath != null && filePath.endsWith('.tflite')) {
        prefs = await SharedPreferences.getInstance();
        await prefs.setString('tfliteFilePath', filePath);
  ...
  Future<void> _clearTfliteFilePath() async {
    prefs = await SharedPreferences.getInstance();
    await prefs.remove('tfliteFilePath');
  ...
  Future<void> _clearLabelFilePath() async {
    prefs = await SharedPreferences.getInstance();
    await prefs.remove('labelFilePath');
  ...
  Future<void> _pickLabelFile() async {
    ...
    if (result != null) {
      ...
      if (filePath != null && filePath.endsWith('.txt')) {
        prefs = await SharedPreferences.getInstance();
        await prefs.setString('labelFilePath', filePath);
  ...
  void _onContinue() async {
    ...
    final isValid = await validator.validateAndNavigate(
      ...
      onValidationSuccess: () async {
        ...
        try {
          // Attempt navigation
          await Navigator.pushNamed(context, '/red');
        } catch (e) {
          // Navigate back and display error message
          if (mounted) {
